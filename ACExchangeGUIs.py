...
class ReadData():
    ...
    def read_data(self,*args):
        ...
        try:
            ...
            self.data = self.data.dropna(subset=["Preferred country 1","Study Field"])
            self.popup.destroy()
...
class InterestFrame():
    def __init__(self,data):
        ...
        self.data = data
        ###
        #Define study fields and regions to test form
        ###
        fields = ["-","Agricult","Archit","Biol","Biochem","Biomedic","Biotech","Chem",
                  "Civil","Computer","Design","Electric","Electromechanic","Energy",
                  "Environment","Industrial","Material","Math","Food technology","Physics"]
        regions = ["-","Northern and Western Europe","Central Europe","Mediterranean Europe",
                   "North America","Canada","Central and South America",
                   "Middle East","Africa",
                   "Central Asia","India","East Asia","South-East Asia","Japan","Oceania"]
        ...
        ###
        self.explain = tk.Label(self.check_interest_frame,text='Enter the country/region and field of the internship you want to check interest for (case sensitive!)')
        ...
        self.study_out.set("-")
        self.entry_study_out = tk.OptionMenu(self.check_interest_frame, self.study_out,*fields)
        self.entry_study_out.grid(column=2,row=2,sticky="ew")

        self.label_country_out = tk.Label(self.check_interest_frame, text='Region')
        self.label_country_out.grid(column=1,row=3,sticky="w")
        self.country_out = tk.StringVar()
        self.country_out.set("-")
        self.entry_country_out = tk.OptionMenu(self.check_interest_frame, self.country_out,*regions)
        self.entry_country_out.grid(column=2,row=3,sticky="ew")
        ...
        self.output_levels.grid(column=2,row=8,sticky="ew")

    def check_interest(self):
        dataset = self.data
        country_ = self.country_out.get()
        study_ = self.study_out.get()
        if study_ == '-' and country_ == '-':
            ...
            #mask = dataset.isin(values_countries)['Preferred country 1']#.any(1)
            data = dataset[dataset['Preferred country 1'].str.contains(country_)]
            #data = dataset[mask]
            if data.empty:
                messagebox.showwarning(title='No candidates',
        ...
        self.output_levels.config(text=', \n'.join([x for x in levels]))
...
class AddOfferFrame():
    ...
    def add_offer(self,*args):
        ...
        if not os.path.isfile(to_read):
            if messagebox.askyesno(title="File doesn't exist yet",message="The filename or the location you entered does not exist yet. Do you want to create it?"):
        ...
        ###
            ...
            messagebox.showinfo(title='Offer added',
    ...
    def plot_studies(self):
        ...
        to_read = str(self.exchanged_offers_filepath.get())
        if not os.path.isfile(to_read):
            messagebox.showwarning(title="File doesn't exist",message="The filename or the location you entered does not exist!")
            return None
        else:
            self.exchanged = pd.read_csv(to_read,sep=',',usecols=['domestic offer code','foreign offer code','country','field','min duration','max duration'])

        if self.exchanged.empty:
            messagebox.showwarning(title="No available data",
            ...
            ###
            studies=['Bio','Environment','Mechanic','Electric','Physic','Civil','Chem','Material','Computer','Architecture']
